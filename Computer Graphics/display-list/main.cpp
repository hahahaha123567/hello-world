...
void DrawTable()
{
	...
	glScalef(tableLength, 1, tableLength);
	glutSolidCube(1.0);
	glPopMatrix();
...
void DrawScene()
{
	for (int i = 0; i < bunnyCount/tableLength+1; ++i) {
		for (int j = 0; j < tableLength && i*tableLength+j<bunnyCount; ++j) {
			glPushMatrix();
			glTranslatef((tableLength/2.0-j)*0.8-0.3, 4.5, (tableLength/2.0-i)*0.8-0.3);
...
void key(unsigned char k, int x, int y)
{
	...
	case 'i':
		{
			bunnyCount++;
			if (tableLength * tableLength < bunnyCount) {
				tableLength++;
			}
			break;
		}
	case 'k': 
		{
			...
			if (tableLength > 4 && (tableLength-1) * (tableLength-1) >= bunnyCount) {
				tableLength--;
			}
			break;
		}
